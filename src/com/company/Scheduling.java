package com.company;// This file contains the main() function for the Scheduling
    ...
    try {   
      ...
      while ((line = in.readLine()) != null) {
        ...
        if (line.startsWith("process")) {
          ...
          processVector.addElement(new sProcess(cputime, ioblocking, 0, 0, 0, curProcess++));
        }
  ...
  public static void main(String[] args) {
    ...
    if (processVector.size() < processnum) {
      ...
      while (processVector.size() < processnum) {       
        ...
        processVector.addElement(new sProcess(cputime,i*100,0,0,0, i));
        i++;
      }
    }
    ...
    try {
      ...
      out.println("Pr - Process\nT - Time\nB - Blocked\nPre - Preemption");
      out.println("Pr #\t\tCPU T\t\tIO B\t\tCPU C\t\tCPU B\t\t\tPree\t\tRunning time");
      for (i = 0; i < processVector.size(); i++) {
        ...
        if (process.numblocked < 10)
          ...
          out.print("\t\t");
        out.print(process.preemted + " times");
        if (process.preemted < 10)
          out.print("\t\t");
        else
          out.print("\t");
        out.println(process.time + " (ms)");
      }
      out.close();
    } catch (IOException e) { /* Handle exceptions */ }
